...
class RoadObjectDetectionImage(QWidget):
    ...
    def predict(self):
        path, _ = QFileDialog.getOpenFileName(self, '이미지 선택', "../classification_date/", "Image Files (*.png *.jpg)")
        if path != '':
            pixmap = QPixmap(path)
...
class RoadObjectDetectionVideo(QWidget):
    ...
    def video_process(self):
        video_path, _ = QFileDialog.getOpenFileName(self, '', "../data/videos", "Video Files (*.mp4 *.mov *.avi *.wmv)")
        if video_path != '':
